# Привет, я Илья! 👋
### Junior Android Developer | Выпускник Ecole 42

Рад видеть вас в моём профиле! Я создаю современные мобильные приложения под Android на языке Kotlin, проектирую плавные UI-интерфейсы и кастомные анимации. Мой бэкграунд в низкоуровневом и системном программировании (Ecole 42) помогает мне писать оптимизированный, чистый и производительный код.

---

## 🛠️ Технологический стек

- **Языки:** Kotlin, Java (Core, Collections, ООП), C/C++ (алгоритмическая база)
- **Архитектура:** MVVM, Clean Architecture, MVI
- **UI & Анимации:** Custom Views (компоненты с нуля), Jetpack Compose, XML Layouts,
ViewBinding, сложные UI-анимации.
- **Асинхронность и многопоточность:** Kotlin Coroutines, Flow
- **Управление зависимостями:** DI (Hilt, Dagger 2)
- **Сеть и БД:** Retrofit, Room DB, OkHttp, Serialization (Moshi / KotlinX Serialization)
- **Инструменты:** Git, Android Studio, Gradle, Intellij IDEA

---

## 📱 Основные проекты

### 🛠️ [NMedia](../../../NMedia) — Мобильное приложение социальной сети
*Клиент-серверное Android-приложение для публикации постов, взаимодействия с контентом и работы с медиа.*
- **Стек:** Kotlin, Coroutines, Room DB, Retrofit, Architecture Components (ViewModel, LiveData).
- **Что сделано:** Реализовал архитектуру MVVM, настроил локальное кэширование через Room для обеспечения offline-first режима и бесшовную синхронизацию данных с удаленным сервером через REST API.

### 🎨 [StatsView](../../../StatsView) — Кастомный UI-компонент с анимацией
*Проект-демонстрация продвинутой работы с графической системой Android и отрисовкой кастомного интерфейса.*
- **Стек:** Kotlin, Canvas API, Android Animation Framework.
- **Что сделано:** С нуля спроектировал и запрограммировал кастомный графический элемент (`Custom View`) для отображения круговой статистики. Реализовал математический расчет секторов и плавные анимации появления данных.

### ☕ [HW_netology](../../../HW_netology) — Проекты на Java Core
*Комплекс практических задач, подтверждающий знание фундаментальной базы и промышленного ООП.*
- **Стек:** Java SE, Collections, Исключения, Алгоритмы фильтрации данных.
- **Что сделано:** Закрепил на практике принципы ООП, реализовал обработку ошибок и работу со сложными структурами данных (`Lists`, `Queues`, `HashMaps`) для эффективной обработки бизнес-логики.

---

## 🎓 Образование и Сертификаты

### 🚀 Нетология (2025 – настоящее время)
**Профессиональный курс: Android-разработчик**
- Специализируюсь на проектировании, разработке и поддержке современных мобильных приложений под платформу Android.
- На практике осваиваю построение современных UI-интерфейсов (как на Jetpack Compose, так и на View-подходе) и внедрение многопоточных архитектурных решений.


### 🏅 Ecole 42 / Школа 21 (Выпуск: 21 июня 2023 г.)
Успешно завершил основное обучение со званием **Architect in Digital Technologies**. 
- Программа построена на жесткой системе *peer-to-peer* (взаимная проверка без лекторов), что развило во мне абсолютную автономность в поиске решений и разборе сложного ТЗ.
- Глубоко освоил низкоуровневые алгоритмы, управление памятью, многопоточность и устройство операционных систем.

> 📄 **[Посмотреть официальный сертификат об окончании Ecole 42](https://github.com/f3ilya/f3ilya/blob/main/certificate_snakita.pdf)**

---

## 📈 Мой прогресс в Ecole 42 (Core)

| No  | Project                                    | Status | No  | Project                                                 | Status |
| :-: | :----------------------------------------- | :----: | :-: | :------------------------------------------------------ | :----: |
| 1   | [libft](../../../libft)                    | ✅     | 10  | [cub3d](../../../cub3D)                                 | ✅     |
| 2   | [Born2beroot](../../../Born2beRoot)        | ✅     | 11  | [miniRT](../../../minirt)                               | ✅     |
| 3   | [ft_printf](../../../ft_printf)            | ✅     | 12  | [CPP Module 00](../../../cpp_modules/tree/main/module00)| ✅     |
| 4   | [get_next_line](../../../get_next_line)    | ✅     | 13  | [CPP Module 01](../../../cpp_modules/tree/main/module01)| ✅     |
| -   | FdF                                        | 🔒     | 14  | [CPP Module 02](../../../cpp_modules/tree/main/module02)| ✅     |
| 5   | [so_long](../../../so_long)                | ✅     | 15  | [CPP Module 03](../../../cpp_modules/tree/main/module03)| ✅     |
| -   | minitalk                                   | 🔒     | 16  | [CPP Module 04](../../../cpp_modules/tree/main/module04)| ✅     |
| 6   | [pipex](../../../pipex)                    | ✅     | 17  | [CPP Module 05](../../../cpp_modules/tree/main/module05)| ✅     |
| 7   | [push_swap](../../../push_swap)            | ✅     | 18  | [CPP Module 06](../../../cpp_modules/tree/main/module06)| ✅     |
| 8   | [minishell](../../../minishell)            | ✅     | 19  | [CPP Module 07](../../../cpp_modules/tree/main/module07)| ✅     |
| 9   | [philosophers](../../../philosophers)      | ✅     | 20  | [CPP Module 08](../../../cpp_modules/tree/main/module08)| ✅     |



---

## 📫 Как со мной связаться

<a href="https://t.me/f3_ilya">
  <img align="left" alt="f3ilya's Telegram" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/telegram.svg" />
</a>
<a href="https://instagram.com/f3_ilya/">
  <img align="left" alt="f3ilya's Instagram" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/instagram.svg" />
</a>
<a href="https://vk.com/f3_ilya/">
  <img align="left" alt="f3ilya's Instagram" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@3.13.0/icons/vk.svg" />
</a>
<a href="mailto:ilya-paranin@mail.ru">
  <img align="left" alt="f3ilya's Mail" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@3.13.0/icons/mail-dot-ru.svg" />
</a>


<!--
**f3ilya/f3ilya** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
